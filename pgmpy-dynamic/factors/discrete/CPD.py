...
class TabularCPD(DiscreteFactor):
    ...
    def _make_table_str_B(self, tablefmt="fancy_grid", print_state_names=True):
        headers_list = []
    ...
    #--------------------------------------------------------------------------------------------------------------------
    """ Extending the _make_table_str function to print CPD tables of dynamic Bayesian networks. 
    ...
    def _make_table_str(self, tablefmt="fancy_grid", print_state_names=True):
        ...
        evidence_card = self.cardinality[1:]
        if evidence:
            if isinstance(evidence[0], tuple):
                col_indexes = np.array(list(product(*[range(i) for i in evidence_card])))

                if self.state_names and print_state_names:
                    for i in range(len(evidence_card)):
                        column_header = [evidence[i]] + ['{var}_{temp}({state})'.format
                                                         (var=evidence[i][0],
                                                          temp=evidence[i][1],
                                                          state=self.state_names[evidence[i][0]][d])
                                                          for d in col_indexes.T[i]]
                        headers_list.append(column_header)
                else:
                    for i in range(len(evidence_card)):
