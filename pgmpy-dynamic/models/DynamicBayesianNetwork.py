...
class DynamicBayesianNetwork(DirectedGraph):
    ...
    def add_node(self, node, temp, **attr):
        """
        ...
        ['A']
        """
        super(DynamicBayesianNetwork, self).add_node((node, temp), **attr)

    #--------------------------------------------------------------------------------------------------------------------
    """ Extending the add_nodes_from function to add nodes in transition Network (times 0 and 1). 
    ...
    def get_cpds(self, node=None, time_slice=1):
        ...
        if node:
            ...
            #sla = [cpd for cpd in self.cpds if set(list(cpd.variables)).issubset(self.get_slice_nodes(time_slice))]
            #print sla
    ...
    def get_parents_dynamic(self, model, node, temp):
        parents = []
        parents = model.get_parents((node,temp))
        return parents
...
#----------------------------------------------------------------------------------------------
# Implementation of fit function for Dynamic Bayesian Networks 
# Contribution by Mariana C. Souza 
# Reference: Learning the Structure of Dynamic Probabilistic Networks (Nir Friedman Kevin Murphy Stuart Russell, 1998) 
#----------------------------------------------------------------------------------------------
    def fit(self, data, estimator=None, state_names=[], complete_samples_only=True, **kwargs):
        ...
        >>>     transitionModel.add_node(node, 0)
        >>>     transitionModel.add_node(node, 1)
        ...
        """
        from pgmpy.estimators import BaseEstimator, BayesianEstimator
        ...
        estimator = estimator(self, data, state_names=state_names,
                             complete_samples_only=complete_samples_only)

        cpds_list = estimator.get_parameters_dynamic(**kwargs)
